...
import axios from "axios";
import React, { useState, useEffect } from "react";
import { Category, ParentCategory } from "@/types/catagory";
...
export default function CategoryModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios({ method, url, data: formPayload });
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              value={categoryType}
              onChange={(e) => setCategoryType(e.target.value as "parent" | "sub")}
              className="w-full border rounded p-2"
            >
              <option value="parent">Parent Category</option>
              <option value="sub">Sub Category</option>
          ...
          <div>
            ...
            {formData.category_image && (
              <Image
                src={
                  typeof formData.category_image === "string"
                    ? `http://localhost:8080${formData.category_image}`
