...
const Chatbot: React.FC<ChatbotProps> = ({
  ...
  const showWelcomeMessage = () => {
    const welcomeMessage: Message = {
      id: generateId(),
      type: 'bot',
      content: `Hi there! 👋 Welcome to ${companyName}. I'm powered by Gemini AI and here to help with your questions. How can I assist you today?`,
      timestamp: Date.now(),
      options: [
  ...
  const TypingIndicator = () => (
    <div className="flex items-center gap-3 mb-4">
      ...
      <div className="bg-gray-100 rounded-2xl px-4 py-3">
        ...
        <div className="text-xs text-gray-500 mt-1">Gemini is thinking...</div>
      </div>
    </div>
  );
          ...
          <div className="mt-3 space-y-2">
            {message.options.map((option, index) => (
              <button
                ...
                className="block w-full text-left px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg hover:bg-gray-50 hover:border-blue-300 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {option}
              </button>
            ))}
    ...
    <div className={`fixed ${positionClasses} z-50`}>
      ...
      {isOpen && (
        <div className={`absolute ${chatPosition} w-96 h-[500px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col animate-in slide-in-from-bottom-5 duration-300 max-w-[calc(100vw-2rem)] sm:w-96`}>
          {/* Header */}
          <div className="bg-blue-500 text-white p-5 rounded-t-2xl flex items-center gap-3">
            <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-blue-500 font-bold text-sm">
              🤖
            </div>
            <div className="flex-1">
              <h3 className="font-semibold text-base">{companyName}</h3>
            ...
            <button
              onClick={clearConversation}
              className="p-1 hover:bg-blue-600 rounded transition-colors text-xs px-2"
              title="Clear conversation"
            >
              Clear
            </button>
          ...
          {error && (
            <div className="px-4 py-2 bg-red-50 border-t border-red-200">
              <p className="text-xs text-red-600">Debug: {error}</p>
            </div>
          )}
